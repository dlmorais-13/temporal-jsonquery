...
```text
npm install temporal-jsonquery
```
...
```js
import { jsonquery } from 'temporal-jsonquery'
...
- `options` is an optional object that can contain the following properties:
  ...
  - `operators` is an optional array definitions for custom operators. Each definition describes the new operator, the name of the function that it maps to, and the desired precedence of the operator: the same, before, or after one of the existing operators (`at`, `before`, or `after`):
    ...
    The defined operators can be used in a text query. Only operators with both a left and right hand side are supported, like `a == b`. They can only be executed when there is a corresponding function. For example:
      ...
      ```js
      import { buildFunction } from 'temporal-jsonquery'
...
```js
import { compile } from 'temporal-jsonquery'
...
```js
import { parse } from 'temporal-jsonquery'
...
```js
import { stringify } from 'temporal-jsonquery'
...
```ts
import { jsonquery, buildFunction } from 'temporal-jsonquery'
...
When `temporal-jsonquery` is installed globally using npm, it can be used on the command line. To install `temporal-jsonquery` globally:

```bash
$ npm install -g temporal-jsonquery
```

Usage:

```
$ temporal-jsonquery [query] {OPTIONS}
```
...
```
$ temporal-jsonquery --input users.json 'sort(.age)'
$ temporal-jsonquery --input users.json 'filter(.city == "Rotterdam") | sort(.age)'
$ temporal-jsonquery --input users.json 'sort(.age)' > output.json
$ temporal-jsonquery --input users.json 'sort(.age)' --output output.json
$ temporal-jsonquery --input users.json --query query.txt
$ temporal-jsonquery --input users.json --query query.json --format json
$ cat users.json | temporal-jsonquery 'sort(.age)'
$ cat users.json | temporal-jsonquery 'sort(.age)' > output.json
```
